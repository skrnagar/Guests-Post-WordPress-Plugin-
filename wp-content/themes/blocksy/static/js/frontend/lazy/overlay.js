...
if (window.ctguest && window.ctguest.focusLock) {
    focusLockToUse = ctguest.focusLock
} else {
    window.ctguest = window.ctguest || {}
    window.ctguest.focusLock = focusLockToUse
}
        ...
        if (!settings.container.hasListener) {
            ...
            settings.container.addEventListener('click', (event) => {
                if (event.target && event.target.matches('a')) {
                    hideOffcanvas(settings, {
                        closeInstant: event.target.getAttribute('href')[0] !== '#',
                    })
                }
            })
        }
