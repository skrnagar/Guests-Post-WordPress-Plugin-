...
export const mount = () => {
    ...
    $(document.body).on('adding_to_cart', () => [...document.querySelectorAll(selector)].map((cart) => {
    ...
    $(document.body).on('wc_fragments_loaded', () => {
        ...
        setTimeout(() => ctEvents.trigger('blocksy:guest:init'))
        scheduleLoad()
    })
    ...
    $(document.body).on('removed_from_cart', (_, __, ___, button) => [...document.querySelectorAll(selector)].map((cart) => {
        if (!button) return
