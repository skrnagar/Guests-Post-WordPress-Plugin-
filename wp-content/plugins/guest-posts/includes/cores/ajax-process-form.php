...
if ( $this->admin_ajax_nonce_verify() ) {
    ...
    if ( empty( $form_row ) ) {
        die( esc_html__( 'No form found for this alias.', 'guest-posts-submission' ) );
    }
    ...
    if ( !empty( $form_fields ) ) {
        ...
        foreach ( $form_fields as $field_key => $field_details ) {
            ...
            if ( !empty( $field_details['show_on_form'] ) ) {
                $required_message = (!empty( $field_details['required_error_message'] )) ? esc_html__( $field_details['required_error_message'] ) : esc_html__( 'This field is requied', 'guest-posts-submission' );
                        ...
                        case 'author_email':
                            if ( !empty( $field_details['character_limit'] ) ) {
                                ...
                                if ( $field_value_length > $field_details['character_limit'] ) {
                                    $character_limit_error_message = (!empty( $field_details['character_limit_error_message'] )) ? esc_html__( $field_details['character_limit_error_message'] ) : esc_html__( sprintf( 'Max characters allowed is %d', $field_details['character_limit'] ), 'guest-posts-submission' );
                                    $error_flag = 1;
        ...
        if ( !empty( $form_details['security']['frontend_form_captcha'] ) ) {
            ...
            $required = esc_html__( 'This field is required', 'guest-posts-submission' );
            if ( empty( $captcha ) ) {
                ...
                if ( is_wp_error( $captcha_response ) ) {
                    $error_details['security'] = esc_html__( 'Captcha Validation failed.', 'guest-posts-submission' );
                    $error_flag = 1;
                } else {
        ...
        if ( $error_flag == 1 ) {
            ...
            $response['message'] = (!empty( $form_details['basic']['validation_error_message'] )) ? esc_html( $form_details['basic']['validation_error_message'] ) : esc_html__( 'Form validation error occurred.', 'guest-posts-submission' );
        } else {
            ...
            if ( $form_row->form_type == 'login_require' ) {
                ...
                if ( is_user_logged_in() ) {
                    ...
                    $response['status'] = 403;
                    $response['message'] = esc_html__( 'Invalid form submission', 'guest-posts-submission' );
            ...
            if ( !empty( $insert_update_post_id ) ) {
                ...
                $response['status'] = 200;
                $response['message'] = (!empty( $form_details['basic']['form_success_message'] )) ? esc_html( $form_details['basic']['form_success_message'] ) : esc_html__( 'Form submission successful.', 'guest-posts-submission' );
                ...
                $response['status'] = 403;
                $response['message'] = esc_html__( 'There occurred some error.', 'guest-posts-submission' );
            }
        }
    } else {
        $response['status'] = 403;
