<div class="fpsml-login-form-wrapper fpsml-login-form-<?php echo esc_attr($form_template); ?>">
    ...
    $username_label = (!empty($login_settings['username_label'])) ? esc_attr($login_settings['username_label']) : __('Username', 'guest-posts-submission');
    $password_label = (!empty($login_settings['password_label'])) ? esc_attr($login_settings['password_label']) : __('Password', 'guest-posts-submission');
    $login_button_label = (!empty($login_settings['login_button_label'])) ? esc_attr($login_settings['login_button_label']) : __('Login', 'guest-posts-submission');
    $remember_me_label = (!empty($login_settings['remember_me_label'])) ? esc_attr($login_settings['remember_me_label']) : __('Remember', 'guest-posts-submission');
    $login_error_message = (!empty($login_settings['login_error_message'])) ? esc_attr($login_settings['login_error_message']) : __('Invalid username or password.', 'guest-posts-submission');
    global $user_login;
    ...
    if ($login === "failed") {
        ...
        $captcha_error_message = (!empty($form_details['security']['error_message'])) ? $form_details['security']['error_message'] : esc_html__('Invalid Captcha', 'guest-posts-submission');
