...
if ( !class_exists( 'FPSML_Library' ) ) {
    ...
    class FPSML_Library {
        ...
        function get_default_fields( $post_type = 'post', $form_type = 'login_require' ) {
            $default_fields = array( 'post_title' => array( 'show_on_form' => 1, 'field_label' => esc_html__( 'Post Title', 'guest-posts-submission' ) ),
                'post_content' => array( 'show_on_form' => 1, 'field_label' => esc_html__( 'Post Content', 'guest-posts-submission' ), 'editor_type' => 'simple' ),
                'post_image' => array(),
                'post_excerpt' => array()
            );
            if ( $form_type == 'guest' ) {
...
A new post has been submitted via Guest Posts Submission plugin in your %s website. Please find details below:
...
Thank you', get_bloginfo( 'name' ) ), 'guest-posts-submission' );
