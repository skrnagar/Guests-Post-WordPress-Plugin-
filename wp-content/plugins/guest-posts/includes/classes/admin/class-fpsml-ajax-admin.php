    ...
    class FPSML_Ajax_Admin {
        ...
        function process_form_edit() {
            if ($this->admin_ajax_nonce_verify()) {
                ...
                if (empty($form_title) || empty($form_alias)) {
                    $response['status'] = 403;
                    $response['message'] = esc_html__('Form title or Alias cannot be empty.', 'guest-posts-submission');
                } else {
                    if ($fpsml_library_obj->is_alias_available($form_alias, $form_id)) {
                        ...
                        $response['status'] = 200;
                        $response['message'] = esc_html__('Form updated successfully.', 'guest-posts-submission');
                    } else {
                        $response['status'] = 403;
                        $response['message'] = esc_html__('Form alias already used. Please use some other alias.', 'guest-posts-submission');
                    }
                }
        ...
        function save_global_settings() {
            if ($this->admin_ajax_nonce_verify()) {
                ...
                $response['status'] = 200;
                $response['message'] = esc_html__('Settings saved successfully', 'guest-posts-submission');
                die(json_encode($response));
            } else {
                $this->permission_denied();
            }
