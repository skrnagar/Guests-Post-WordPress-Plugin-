        ...
        function media_delete_action() {
            if ($this->admin_ajax_nonce_verify()) {
                ...
                if ($media_key != $attachment_code) {
                    $response['status'] = 403;
                    $response['messsage'] = esc_html__('Unauthorized access', 'guest-posts-submission');
                } else {
                    $media_delete_check = wp_delete_attachment($media_id, true);
                    if ($media_delete_check) {
                        $response['status'] = 200;
                        $response['messsage'] = esc_html__('Media deleted successfully.', 'guest-posts-submission');
                    } else {
                        $response['status'] = 403;
                        $response['messsage'] = esc_html__('Error occurred while deleting the media.', 'guest-posts-submission');
                    }
                }
        ...
        function process_post_delete() {
            if ($this->admin_ajax_nonce_verify()) {
                ...
                if ($delete_key != $verify_delete_key) {
                    $response['status'] = 403;
                    $response['message'] = esc_html__('Unauthorized delete from delete key', 'guest-posts-submission');
                } else {
                    ...
                    if ($current_user_id != $post_author_user_id) {
                        $response['status'] = 403;
                        $response['message'] = esc_html__('Unauthorized delete from user', 'guest-posts-submission');
                    } else {
                        $delete_check = wp_trash_post($post_id);
                        if ($delete_check) {
                            $response['status'] = 200;
                            $response['message'] = esc_html__('Post delete successfully.', 'guest-posts-submission');
                        } else {
                            $response['status'] = 403;
