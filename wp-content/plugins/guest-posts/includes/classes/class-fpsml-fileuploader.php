    ...
    class FPSML_qqFileUploaders {
        ...
        function handleUpload( $uploadDirectory, $replaceOldFile = FALSE, $upload_url ) {
            if ( !is_writable( $uploadDirectory ) ) {
                return array( 'error' => esc_html__( "Server error. Upload directory isn't writable.", 'guest-posts-submission' ) );
            }

            if ( !$this->file ) {
                return array( 'error' => esc_html__( 'No files were uploaded.', 'guest-posts-submission' ) );
            }

            $size = $this->file->getSize();

            if ( $size == 0 ) {
                return array( 'error' => esc_html__( 'File is empty', 'guest-posts-submission' ) );
            }

            if ( $size > $this->sizeLimit ) {
                return array( 'error' => esc_html__( 'File is too large', 'guest-posts-submission' ) );
            }
            ...
            if ( $this->file->save( $uploadDirectory . $filename . '.' . $ext ) ) {
                ...
                return array( 'error' => esc_html__( 'Could not save uploaded file.The upload was cancelled, or server error encountered', 'guest-posts-submission' ) );
            }
        }
