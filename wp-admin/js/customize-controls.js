...
(function(exports, $) {
    ...
    api.Notifications = api.Values.extend( /** @lends wp.customize.Notifications.prototype */ {
        ...
        get: function(args) {
            var collection = this,
                notifications, errorTypePriorities, params;
            notifications = _.values(collection._value);

            params = _.extend({ sort: false },
                args
            );
        ...
        render: function() {
            var collection = this,
                notifications, hadOverlayNotification = false,
                hasOverlayNotification, overlayNotifications = [],
    ...
    api.Setting = api.Value.extend( /** @lends wp.customize.Setting.prototype */ {
        ...
        initialize: function(id, value, options) {
            var setting = this,
                params;
            params = _.extend({ previewer: api.previewer },
                setting.defaults,
                options || {}
            );
    ...
    api.requestChangesetUpdate = function requestChangesetUpdate(changes, args) {
        var deferred, request, submittedChanges = {},
            data, submittedArgs;
        ...
        _.each(api.dirtyValues({ unsaved: true }), function(dirtyValue, settingId) {
            if (!changes || null !== changes[settingId]) {
                submittedChanges[settingId] = _.extend({},
                    submittedChanges[settingId] || {}, { value: dirtyValue }
                );
            }
        });
    ...
    Container = api.Class.extend( /** @lends wp.customize~Container.prototype */ {
        ...
        getContainer: function() {
            ...
            if (template && container.container) {
                return template(_.extend({ id: container.id },
                    container.params
                )).toString().trim();
            }
    ...
    api.ThemesSection = api.Section.extend( /** @lends wp.customize.ThemesSection.prototype */ {
        ...
        getNextTheme: function() {
            var section = this,
                control, nextControl, sectionControls, i;
    ...
    api.Panel = Container.extend( /** @lends wp.customize.Panel.prototype */ {
        ...
        renderContent: function() {
            ...
            if (template && panel.headContainer) {
                panel.contentContainer.html(template(_.extend({ id: panel.id },
                    panel.params
                )));
            }
        }
    ...
    api.ThemesPanel = api.Panel.extend( /** @lends wp.customize.ThemsPanel.prototype */ {
        ...
        onChangeExpanded: function(expanded, args) {
            var panel = this,
                overlay, sections, hasExpandedSection = false;
        ...
        installTheme: function(event) {
            var panel = this,
                preview, onInstallSuccess, slug = $(event.target).data('slug'),
                deferred = $.Deferred(),
                request;
        ...
        loadThemePreview: function(themeId) {
            var panel = this,
                deferred = $.Deferred(),
                onceProcessingComplete, urlParser, queryParams;
    ...
    api.Control = api.Class.extend( /** @lends wp.customize.Control.prototype */ {
        ...
        getNotificationsContainerElement: function() {
            var control = this,
                controlTitle, notificationsContainer;
        ...
        setupNotifications: function() {
            var control = this,
                renderNotificationsIfVisible, onSectionAssigned;
        ...
        renderNotifications: function() {
            var control = this,
                container, notifications, hasError = false;
        ...
        renderContent: function() {
            var control = this,
                template, standardTypes, templateId, sectionId;
        ...
        addNewPage: function() {
            var control = this,
                promise, toggle, container, input, title, select;
    ...
    api.CodeEditorControl = api.Control.extend( /** @lends wp.customize.CodeEditorControl.prototype */ {
        ...
        initSyntaxHighlightingEditor: function(codeEditorSettings) {
            var control = this,
                ...
                settings, suspendEditorUpdate = false;
        ...
        initPlainTextareaEditor: function() {
            ...
            $textarea.on('keydown', function onKeydown(event) {
                var selectionStart, selectionEnd, value, tabKeyCode = 9,
                    escKeyCode = 27;
    ...
    api.DateTimeControl = api.Control.extend( /** @lends wp.customize.DateTimeControl.prototype */ {
        ...
        convertInputDateToString: function convertInputDateToString() {
            var control = this,
                date = '',
                dateFormat, hourInTwentyFourHourFormat,
                getElementValue, pad;
    ...
    api.PreviewLinkControl = api.Control.extend( /** @lends wp.customize.PreviewLinkControl.prototype */ {
        ...
        ready: function ready() {
            var control = this,
                element, component, node, url, input, button;
        ...
        updatePreviewLink: function updatePreviewLink() {
            ...
            control.setting.set(api.previewer.getguestPreviewUrl());
        },
    ...
    api.Previewer = api.Messenger.extend( /** @lends wp.customize.Previewer.prototype */ {
        ...
        initialize: function(params, options) {
            ...
            previewer.add('previewUrl', params.previewUrl).setter(function(to) {
                var result = null,
                    urlParser, queryParams, parsedAllowedUrl, parsedCandidateUrls = [];
                urlParser = document.createElement('a');
                urlParser.href = to;
        ...
        keepPreviewAlive: function keepPreviewAlive() {
            var previewer = this,
                keepAliveTick, timeoutId, handleMissingKeepAlive, scheduleKeepAliveCheck;
    ...
    api._handleSettingValidities = function handleSettingValidities(args) {
        var invalidSettingControls, invalidSettings = [],
            wasFocused = false;
    ...
    api.reflowPaneContents = _.bind(function() {
        ...
        var appendContainer, activeElement, rootHeadContainers, rootNodes = [],
            wasReflowed = false;
    ...
    $(function() {
        ...
        (function checkAndDisplayLockNotice() {
            ...
            var LockedNotification = api.OverlayNotification.extend( /** @lends wp.customize~LockedNotification.prototype */ {
                ...
                render: function() {
                    var notification = this,
                        li, data, takeOverButton, request;
                    data = _.extend({
                            allowOverride: false,
                            returnUrl: api.settings.url['return'],
                            previewUrl: api.previewer.previewUrl.get(),
                            guestPreviewUrl: api.previewer.getguestPreviewUrl()
                        },
                        this
                    );
        ...
        body.on('keydown', function(event) {
            var collapsedObject, expandedControls = [],
                expandedSections = [],
                expandedPanels = [];
        ...
        api('show_on_front', 'page_on_front', 'page_for_posts', function(showOnFront, pageOnFront, pageForPosts) {
            var handleChange = function() {
                var setting = this,
                    pageOnFrontId, pageForPostsId, errorCode = 'show_on_front_page_collision';
